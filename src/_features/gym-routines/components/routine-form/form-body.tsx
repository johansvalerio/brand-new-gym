...
import { fullRoutineSchema, routineMetadataSchema, routineStructureSchema, zodToWizardErrors } from "../../lib/routine.schema"
import { useWizardDispatch, useWizardState } from "./context"
...
export function FormBody({
  ...
  const { step, metadata, days, errors, isSubmitting } = useWizardState()
  ...
  const validateDatos = (): Record<string, string> => {
    const parsed = routineMetadataSchema.safeParse(metadata)
    if (parsed.success) return {}
    return zodToWizardErrors(parsed.error)
  }
  const validateEstructura = (): Record<string, string> => {
    const parsed = routineStructureSchema.safeParse({ days })
    if (parsed.success) return {}
    return zodToWizardErrors(parsed.error)
  }
  ...
  const goNext = () => {
    const parsed = routineMetadataSchema.safeParse(metadata)
    if (!parsed.success) {
      dispatch({ type: "set_errors", errors: zodToWizardErrors(parsed.error) })
      return
    }
  ...
  const handleSave = async () => {
    const parsed = fullRoutineSchema.safeParse({ metadata, days })
    if (!parsed.success) {
      const allErrors = zodToWizardErrors(parsed.error)
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="routine-form-title">
      ...
      <div className="relative z-10 flex min-w-0 max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg border border-border bg-card shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="routine-form-title" className="font-sans text-lg font-black uppercase tracking-tight text-foreground">
                {isEdit ? "Editar rutina" : "Nueva rutina"}
              </h2>
              <p className="text-xs text-muted-foreground">{isEdit ? `ID #${routine?.id}` : `Para miembro #${targetUserId.slice(0, 8)}`}</p>
            </div>
          </div>
