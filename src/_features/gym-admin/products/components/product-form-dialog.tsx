...
import { productFormSchema, zodToFormErrors } from "../../lib/product.schema"
...
export function ProductFormDialog(props: ProductFormDialogProps) {
  const { open, product, categories, onClose, onSubmit } = props
...
function ProductFormInner({
  ...
  onClose,
  onSubmit,
}: Omit<ProductFormDialogProps, "open"> & { product: ProductRow | null }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="product-form-title">
      ...
      <div className="relative z-10 flex max-h-[85vh] w-full max-w-lg flex-col overflow-hidden rounded-lg border border-border bg-card shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-border px-4 py-3 sm:px-6 sm:py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{isEdit ? `ID #${product?.product_id}` : "Agregar al catálogo"}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="relative flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 py-4 sm:px-6 sm:py-5">
          <Field label="Nombre" htmlFor="product_name" error={errors.product_name}>
            <input ref={firstFieldRef} id="product_name" value={form.product_name} onChange={(e) => set("product_name", e.target.value)} placeholder="Whey Protein 2lb" className={inputCls(errors.product_name)} />
          </Field>
          <Field label="Descripción" htmlFor="product_description">
            <textarea id="product_description" value={form.product_description} onChange={(e) => set("product_description", e.target.value)} rows={2} placeholder="Proteína de suero de leche, sabor chocolate." className={`${inputCls()} resize-none`} />
          </Field>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field label="Precio ($)" htmlFor="product_price" error={errors.product_price}>
              <input id="product_price" inputMode="decimal" value={form.product_price} onChange={(e) => set("product_price", e.target.value)} placeholder="45" className={inputCls(errors.product_price)} />
            </Field>
            <Field label="Stock" htmlFor="product_stock" error={errors.product_stock}>
              <input id="product_stock" inputMode="numeric" value={form.product_stock} onChange={(e) => set("product_stock", e.target.value)} placeholder="12" className={inputCls(errors.product_stock)} />
            </Field>
          </div>
          <Field label="URL de imagen (opcional)" htmlFor="product_image">
            <input id="product_image" value={form.product_image} onChange={(e) => set("product_image", e.target.value)} placeholder="https://..." className={inputCls()} />
          </Field>
          <Field label="Categoría (opcional)" htmlFor="category_id">
            <select id="category_id" value={form.category_id} onChange={(e) => set("category_id", e.target.value)} className={inputCls()}>
