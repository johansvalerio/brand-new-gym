...
/** Payload que emite el form (sin slug: lo genera el padre). */
...
export function PlanFormDialog({ open, plan, onClose, onSubmit }: PlanFormDialogProps) {
  ...
  const validate = () => {
    ...
    if (
      ...
      next.duration_days = "Duración inválida (entero ≥ 1)."
    const price = Number(form.price)
    if (form.price === "" || Number.isNaN(price) || price < 0)
      next.price = "Ingresa un precio válido (≥ 0)."
    setErrors(next)
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {isEdit ? `ID ${plan?.slug}` : "Agregar un plan de membresía"}
              </p>
            </div>
          </div>
          <button
          ...
          <label className="flex cursor-pointer items-center gap-3 rounded-md border border-border bg-secondary/30 px-4 py-3">
            ...
            <div>
              ...
              <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">
                Visible para que los miembros lo elijan
              </p>
            </div>
          </label>
