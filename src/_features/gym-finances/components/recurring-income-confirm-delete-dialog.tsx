...
interface RecurringIncomeConfirmDeleteDialogProps {
  income: RecurringIncomeRow | null
  onCancel: () => void
  onConfirm: () => Promise<void>
}

export function RecurringIncomeConfirmDeleteDialog({
  income,
  onCancel,
  onConfirm,
}: RecurringIncomeConfirmDeleteDialogProps) {
  ...
  useBodyScrollLock(Boolean(income))
  ...
  return (
    <div
      ...
      role="alertdialog"
      aria-modal="true"
      aria-labelledby="confirm-delete-rent-title"
    >
      ...
      <div className="relative z-10 max-h-[85vh] w-full max-w-sm overflow-y-auto rounded-lg border border-border bg-card p-4 shadow-2xl sm:p-6">
        ...
        <h2 id="confirm-delete-rent-title" className="font-sans text-lg font-black uppercase tracking-tight text-foreground">
          Eliminar renta
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          {"¿Seguro que deseas eliminar "}
          <span className="font-semibold text-foreground">{income.description}</span>
          {"? Dejará de contar en los meses."}
        </p>
