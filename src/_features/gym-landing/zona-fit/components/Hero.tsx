...
const BAND_PHRASE = 'Tu Zona // Tu Ritmo // Comunidad // Constancia // Energía // Sin Presión // Paso a Paso // Resultados Reales //';
...
/** Hero propio de zona-fit: misma estructura/animación que gym-ulate, voz distinta. */
export function ZonaFitHero() {
  ...
  return (
    ...
    >
      {/* ░░ Background photo — gym claro y comunitario ░░ */}
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1571902943202-507ec2618e8f?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center opacity-[0.30] grayscale contrast-125" />
      ...
      <h1 aria-label="Muévete firme, cada día" className="relative z-10">
        {/* MUÉVETE — solid */}
        <div className="overflow-hidden">
          <span aria-hidden="true" className="h5-word block font-heading font-black uppercase leading-[0.82] tracking-[-0.03em] text-[clamp(4rem,17vw,12.5rem)] lg:text-[length:min(14vw,18vh)] text-foreground cursor-default transition-colors duration-300 hover:text-primary">
            <span className="h5-word-inner block">Muévete</span>
          </span>
        </div>
          ...
          >
            <span className="h5-word-inner block">
              <span className="relative inline-block">
                <span className="h5-glitch-main inline-block">Firme</span>
                <span
                  aria-hidden="true"
        ...
        <div className="overflow-hidden">
          <span aria-hidden="true" className="h5-word block font-heading font-black uppercase leading-[0.85] tracking-[-0.03em] text-[clamp(2.6rem,10.5vw,8rem)] lg:text-[min(9vw,13vh)] pl-[3%] text-primary cursor-default transition-colors duration-300 hover:text-foreground">
            <span className="h5-word-inner block">Cada Día</span>
          </span>
        </div>
      </h1>
      ...
      <div className="h5-foot relative z-10 container mx-auto px-6 mt-10 lg:mt-4">
        <p className="font-mono text-[10px] uppercase tracking-[0.26em] text-muted-foreground border-t border-border/60 pt-5">
          Cañas centro · Lun–Sáb · Tu zona, tu ritmo
        </p>
      </div>
    </section>
  );
