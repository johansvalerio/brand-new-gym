...
const LOCATION_QUESTION = "¿Dónde están ubicados?"
...
export function FaqSection() {
  ...
  // Dirección del gym por slug para la respuesta de ubicación.
  const gym = useGym();
  const { site } = getLanding(gym?.slug ?? "gym-ulate");
  const locationAnswer = `En ${site.address.city}, ${site.address.region} — ${site.address.street}. Ver mapa en la sección Ubicación y Horario.`;
  const faqs = FAQS.map((f) =>
    f.q === LOCATION_QUESTION ? { ...f, a: locationAnswer } : f,
  );
