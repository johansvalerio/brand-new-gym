...
export function Footer() {
  ...
  // Datos del gym por slug (nombre, email, teléfono, dirección). Fallback a gym-ulate.
  const gym = useGym();
  const { site } = getLanding(gym?.slug ?? "gym-ulate");
  const [titleLine1, titleLine2] = site.footerTitle;
  const addressLine = `${site.address.street}, ${site.address.city}, ${site.address.region}, Costa Rica`;
          ...
          <h2 className="font-heading font-black uppercase leading-[0.85] tracking-[-0.03em] text-[clamp(2.75rem,7vw,5.5rem)] text-foreground">
            ...
            <span className="text-primary">
              <span className="relative inline-block">
                <span className="ft-glitch-main inline-block">{titleLine2}</span>
                <span
                  aria-hidden="true"
