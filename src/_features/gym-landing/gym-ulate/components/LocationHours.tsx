...
import { getLanding } from "../../shared/registry";

gsap.registerPlugin(ScrollTrigger);

export function LocationHours() {
  const [now, setNow] = useState(() => new Date());
  // Nombre y dirección del gym por slug (la card decía "Gym Ulate" en todos).
  const gym = useGym();
  const { site } = getLanding(gym?.slug ?? "gym-ulate");
  ...
  useEffect(() => {
    ...
    // Estado inicial ANTES de crear el timeline (evita 1 frame con el pin visible).
    ...
    // Cleanup: matar SOLO el trigger propio. getAll().kill() apagaba
    // los pins del story/hero/finalCTA en remontajes (StrictMode/Fast Refresh).
          ...
          >
            ...
            >
              ...
              <path
                ...
                strokeLinecap="round"
                filter="url(#glow)"
                style={{ opacity: 0.9, stroke: "var(--primary)" }}
              />
              {/* Route dots */}
              <circle cx="100" cy="500" r="6" opacity="0.6" style={{ fill: "var(--primary)" }} />
              <circle cx="300" cy="350" r="6" opacity="0.6" style={{ fill: "var(--primary)" }} />
              <circle cx="500" cy="200" r="6" opacity="0.6" style={{ fill: "var(--primary)" }} />
            </svg>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="font-mono text-muted-foreground text-sm leading-relaxed">
                    {site.address.street}
                    <br />
                    {site.address.city}, {site.address.region}, Costa Rica
                  </p>
                </div>
                <div className="hidden sm:block">
                  <a
                    href="#"
                    className="p-3 rounded-xl bg-secondary/10 text-primary hover:bg-secondary/20 border border-primary/30 transition-all duration-300 group/btn"
