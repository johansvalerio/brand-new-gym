...
export interface CoverflowCarouselProps {
  ...
  cardWidth?: string;
  /** Card height as a multiple of card width (1 = square, 1.25 = 4:5). */
  cardRatio?: number;
      ...
      <div className="relative">
        ...
        >
          <div
            ...
            style={{
              height: `calc(var(--cf-card) * ${cardRatio})`,
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                className={cn(
                  "absolute left-1/2 top-0 overflow-hidden rounded-xl bg-muted shadow-xl will-change-transform",
                  cardClassName,
                )}
                style={{
                  width: "var(--cf-card)",
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[200] -translate-y-1/2 cursor-pointer rounded-full border border-border bg-background/70 p-2 text-foreground backdrop-blur transition hover:border-primary hover:text-primary"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[200] -translate-y-1/2 cursor-pointer rounded-full border border-border bg-background/70 p-2 text-foreground backdrop-blur transition hover:border-primary hover:text-primary"
            >
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-6 w-full max-w-[260px] font-mono text-xs">
              {active.meta.map((row) => (
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "size-2 cursor-pointer rounded-full bg-primary transition-opacity",
                index === selected ? "opacity-100" : "opacity-30 hover:opacity-60",
              )}
            />
          ))}
        </div>
